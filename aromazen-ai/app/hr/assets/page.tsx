...
function RegisterChoice({ active, title, description, count, icon, onClick }: { active: boolean; title: string; description: string; count?: number; icon: React.ReactNode; onClick: () => void }) {
  return <div className="relative"><button type="button" aria-pressed={active} onClick={onClick} className={`group flex min-h-24 items-center gap-4 rounded-2xl border p-4 pr-12 text-left w-full transition-all ${active ? 'border-primary/45 bg-primary/[0.08] shadow-sm ring-1 ring-primary/15' : 'border-border bg-card hover:border-primary/25 hover:bg-muted/30'}`}>
    ...
    <span className="min-w-0 flex-1"><span className="flex items-center justify-between gap-3"><span className="font-semibold">{title}</span><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${active ? 'bg-primary/15 text-primary' : 'bg-muted text-muted-foreground'}`}>{count ?? '—'}</span></span></span>
  </button><span className="absolute right-3 top-1/2 -translate-y-1/2"><InfoTip label={`About ${title}`} align="right">{description}</InfoTip></span></div>
}
...
function AssetInventoryList({ assetGroup, data, busy, activeSummary, onClearSummary, onService, onEdit, onRemove }: {
  ...
  return <section id="asset-inventory-list" className="overflow-hidden rounded-2xl border border-border bg-card">
    <header className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-4 md:px-5">
      <div><h2 className="text-sm font-semibold">{assetGroup === 'IT' ? 'IT asset register' : 'General asset register'}</h2><InfoTip label="About the asset register">Open source details for the full asset record.</InfoTip></div>
...
function CustomFieldsEditor({ value, onChange }: { value: Record<string, string>; onChange: (value: Record<string, string>) => void }) { const entries = Object.entries(value); function update(index: number, key: string, fieldValue: string) { const next = [...entries]; next[index] = [key, fieldValue]; onChange(Object.fromEntries(next.filter(([name]) => name.trim()))) } return <div><div className="flex items-center justify-between gap-3"><span className="text-xs text-muted-foreground">Equipment-specific source details</span><button type="button" onClick={() => onChange({ ...value, [`Detail ${entries.length + 1}`]: '' })} className="shrink-0 text-xs font-medium text-primary">+ Add detail</button></div><div className="mt-2 space-y-2">{entries.length === 0 ? <InfoTip label="Additional asset details">Add details such as tonnage, capacity, engine number, insurance period or operating status.</InfoTip> : entries.map(([key, fieldValue], index) => <div key={`${key}-${index}`} className="grid grid-cols-[minmax(0,1fr)_auto] gap-2 sm:grid-cols-[minmax(120px,.7fr)_1fr_auto]"><input value={key} onChange={(event) => update(index, event.target.value, fieldValue)} placeholder="Detail name" className="h-10 min-w-0 rounded-lg border border-border bg-background px-3 text-sm" /><input value={fieldValue} onChange={(event) => update(index, key, event.target.value)} placeholder="Value" className="h-10 min-w-0 rounded-lg border border-border bg-background px-3 text-sm max-sm:row-start-2" /><button type="button" onClick={() => onChange(Object.fromEntries(entries.filter((_, itemIndex) => itemIndex !== index)))} className="row-span-2 rounded-lg border border-border px-3 text-red-400 sm:row-span-1" aria-label={`Remove ${key || 'detail'}`}><X className="h-4 w-4" /></button></div>)}</div></div> }
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) { return <div className="fixed inset-0 z-50 grid place-items-center bg-black/65 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={title}><section className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl border border-border bg-card shadow-2xl"><header className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-border bg-card px-5 py-4"><h2 className="min-w-0 break-words text-lg font-semibold [overflow-wrap:anywhere]">{title}</h2><button type="button" onClick={onClose} className="shrink-0 rounded-lg p-2 text-muted-foreground hover:bg-muted hover:text-foreground" aria-label="Close dialog"><X className="h-4 w-4" /></button></header><div className="p-5">{children}</div></section></div> }
