...
export function RegulatoryDocumentsTool() {
  ...
  function reportResearch(result: RegulatoryWorkflow) {
    ...
    if (summary.populated) notify('success', `AI filled regulatory data for ${summary.populated} of ${summary.attempted} ingredient${summary.attempted === 1 ? '' : 's'}. Review the highlighted rows.`)
    else notify('warning', `AI completed the search but found no verified official matches for ${summary.attempted} ingredient${summary.attempted === 1 ? '' : 's'}.`)
  }
  ...
  return <main className="space-y-5 p-4 md:p-6">
    ...
    {!workflow ? <><section className="grid gap-4 rounded-2xl border border-border bg-card p-4 md:grid-cols-2"><UploadBox title="Regulatory Excel" file={formula} accept=".xlsx,.xlsm" onChange={setFormula} /><UploadBox title="Creation COA" file={coa} accept=".pdf,.docx,.xlsx" onChange={setCoa} /></section><div className="flex justify-end"><Button disabled={!formula || !coa || busy === 'intake'} onClick={() => void start()}>{busy === 'intake' ? <LoaderCircle className="mr-2 h-4 w-4 animate-spin" /> : <FileSpreadsheet className="mr-2 h-4 w-4" />}Process files</Button></div></> : <>
      ...
      <section className="rounded-2xl border border-border bg-card p-4 md:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-semibold">SDS composition and regulatory review</h2><p className="text-xs text-muted-foreground">Source and review information stays in this screen only and is never printed.</p></div>{!locked ? <div className="flex gap-2"><Button variant="outline" disabled={busy === 'enrich'} onClick={() => void enrich()}>{busy === 'enrich' ? <LoaderCircle className="mr-2 h-4 w-4 animate-spin" /> : <Search className="mr-2 h-4 w-4" />}{busy === 'enrich' ? 'AI researching ingredients…' : researchSummary ? 'Retry AI research' : 'Research missing data'}</Button><Button variant="outline" disabled={busy === 'enrich'} onClick={() => setRows((current) => [...current, blankIngredient()])}><Plus className="mr-2 h-4 w-4" />Add ingredient</Button></div> : null}</div>
        {busy === 'enrich' ? <div className="mt-4 rounded-xl border border-primary/30 bg-primary/5 p-3 text-sm text-primary"><LoaderCircle className="mr-2 inline h-4 w-4 animate-spin" />Searching official sources separately for every ingredient. Please keep this page open.</div> : researchSummary ? <div className={`mt-4 rounded-xl border p-3 text-sm ${researchSummary.populated ? 'border-emerald-500/30 bg-emerald-500/5 text-emerald-300' : 'border-amber-500/30 bg-amber-500/5 text-amber-300'}`}>AI populated {researchSummary.populated} of {researchSummary.attempted} researched ingredients. {researchSummary.unresolved} still require employee input.{researchSummary.failed ? ` ${researchSummary.failed} searches failed and can be retried.` : ''}</div> : null}
