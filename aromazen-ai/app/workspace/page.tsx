...
export default function WorkspacePage() {
  ...
  async function openCitation(citation: { documentId: string; collectionId: string }) {
  ...
  return <AppLayout><div className="flex h-full"><div className="flex-1 flex flex-col overflow-hidden"><header className="h-14 border-b border-border bg-card/50 px-6 flex items-center"><span className="w-2 h-2 rounded-full bg-emerald-500 mr-2" /><span className="text-sm font-medium text-foreground">AROMAZEN AI</span><span className="text-xs text-muted-foreground ml-2">Permission-aware streaming workspace</span></header><div className="flex-1 overflow-y-auto px-6 py-6">{messages.length === 0 ? <div className="space-y-8 max-w-2xl mx-auto pt-12"><div className="text-center space-y-3"><h1 className="text-3xl font-semibold text-foreground">What can I help you with?</h1><p className="text-muted-foreground">Ask about knowledge you are allowed to access.</p></div><PromptSuggestions suggestions={suggestions} onSelect={(text) => void sendMessage(text)} /></div> : <div className="max-w-3xl mx-auto space-y-6">{messages.map((message, index) => <ChatMessage key={message.id} role={message.role} content={message.content} timestamp={new Date(message.created_at)} status={message.role === 'assistant' && index === messages.length - 1 ? stage : null} sources={message.citations.map((citation) => ({ documentId: citation.document_id, collectionId: citation.collection_id, name: citation.document_name, collection: citation.collection_name, page: citation.page ?? undefined, relevance: citation.relevance ?? 0 }))} onOpenSource={(source) => void openCitation(source)} />)}</div>}</div><ChatComposer disabled={isSending} onSend={sendMessage} /></div></div></AppLayout>
}
