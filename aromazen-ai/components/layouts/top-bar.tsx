...
import { AlertTriangle, ArrowLeft, Bell, BookOpen, Boxes, Check, CheckCheck, LayoutDashboard, Menu, MessageSquare, Search, Send, Settings, Shield, TrendingUp, Wrench, X } from 'lucide-react'
...
export function TopBar({ sidebarOpen, onSidebarToggle }: TopBarProps) {
  ...
  useEffect(() => {
    const close = (event: MouseEvent) => {
      if ((event.target as Element).closest?.('[data-viewport-overlay]')) return
      if (!shellRef.current?.contains(event.target as Node)) { setSearchOpen(false); setNotificationsOpen(false) }
    }
  ...
  const results = useMemo(() => {
    const navigation: SearchItem[] = [
      ...
      ...(['Marketing', 'Merchandising'].includes(user?.department_name ?? '') || user?.role_names.some((role) => role === 'Super Admin' || role === 'Admin') ? [{ title: 'Marketing Lead Handover', subtitle: 'Submit or review leads sent to Merchandising', href: '/department-tools/marketing-leads', icon: <Send /> }] : []),
  ...
  return <header className="top-bar relative z-20 flex h-[60px] min-w-0 items-center gap-2 border-b border-border bg-background/80 px-3 backdrop-blur-xl sm:h-[68px] sm:gap-4 sm:px-4 md:px-7" ref={shellRef}>
    ...
    <div className="relative">
      ...
      {notificationsOpen && <ViewportOverlay label="Notifications" onClose={() => setNotificationsOpen(false)} closeOnBackdrop><div className="max-h-[calc(100dvh-2rem)] w-full max-w-[400px] overflow-hidden rounded-2xl border border-border bg-popover shadow-2xl"><div className="flex items-start justify-between gap-3 border-b border-border px-4 py-3"><div className="min-w-0"><p className="text-sm font-semibold">Notifications</p><p className="text-xs text-muted-foreground">Lead, knowledge, document, asset and usage updates</p></div><div className="flex shrink-0 items-center gap-1">{unreadCount > 0 ? <button type="button" onClick={() => void markAllRead()} className="inline-flex items-center gap-1.5 rounded-lg px-2 py-1 text-xs font-medium text-primary hover:bg-muted"><CheckCheck className="h-3.5 w-3.5" />Mark all read</button> : null}<button type="button" onClick={() => setNotificationsOpen(false)} className="grid h-8 w-8 place-items-center rounded-lg text-muted-foreground hover:bg-muted hover:text-foreground" aria-label="Close notifications"><X className="h-4 w-4" /></button></div></div><div className="max-h-[min(24rem,calc(100dvh-10rem))] overflow-y-auto p-2">{notifications.length ? notifications.map((item) => <div key={item.id} className={`flex gap-2 rounded-xl p-2 transition-colors hover:bg-muted ${item.is_read ? 'opacity-65' : 'bg-primary/[0.04]'}`}><button type="button" onClick={() => void openNotification(item)} className="flex min-w-0 flex-1 gap-3 p-1 text-left"><span className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${item.severity === 'critical' ? 'bg-destructive/10 text-destructive' : item.severity === 'warning' ? 'bg-amber-500/10 text-amber-600' : 'bg-primary/10 text-primary'}`}>{item.kind === 'document_reminder' || item.kind === 'knowledge_document_added' ? <BookOpen className="h-4 w-4" /> : item.kind === 'asset_maintenance' ? <Wrench className="h-4 w-4" /> : item.kind?.startsWith('marketing_lead_') ? <Send className="h-4 w-4" /> : <AlertTriangle className="h-4 w-4" />}</span><span className="min-w-0"><span className="block text-sm font-medium">{item.title}</span><span className="mt-1 block text-xs leading-5 text-muted-foreground">{item.message}</span></span></button>{!item.is_read ? <button type="button" onClick={() => void markRead(item)} className="mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-muted-foreground hover:bg-background hover:text-primary" aria-label={`Mark ${item.title} as read`} title="Mark as read"><Check className="h-4 w-4" /></button> : null}</div>) : <div className="px-5 py-10 text-center"><Bell className="mx-auto h-5 w-5 text-muted-foreground" /><p className="mt-3 text-sm font-medium">No notifications yet</p><p className="mt-1 text-xs text-muted-foreground">Lead, knowledge, maintenance and usage updates will appear here.</p></div>}</div>{hasPermission('settings.manage') && <button onClick={() => go('/settings#usage-alerts')} className="w-full border-t border-border px-4 py-3 text-left text-xs font-medium text-primary hover:bg-muted">Manage usage alert limits</button>}</div></ViewportOverlay>}
    </div>
  </header>
}
