...
import { FileText, Image as ImageIcon, Library, Loader2, Paperclip, Send, X, Zap } from 'lucide-react'
...
interface ChatComposerProps {
  disabled?: boolean
  onSend: (message: string, attachments: ChatAttachment[], mode: 'chat' | 'image') => Promise<boolean>
...
export function ChatComposer({ disabled = false, onSend, onUpload, collections = [], knowledgeScope = 'auto', onKnowledgeScopeChange }: ChatComposerProps) {
  ...
  return <div className="shrink-0 bg-background px-3 pb-4 pt-2 md:px-6">
    <div className="mx-auto max-w-3xl">
      <div className="rounded-[26px] border border-border bg-card shadow-[0_8px_32px_rgba(0,0,0,0.16)] transition-colors focus-within:border-primary/50">
        ...
        <textarea value={message} onChange={(event) => setMessage(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); void handleSend() } }} disabled={disabled} placeholder={mode === 'image' ? 'Describe the image you want to create…' : attachments.length ? 'Ask anything about your attached files…' : 'Message AROMAZEN AI…'} className="max-h-40 min-h-14 w-full resize-none bg-transparent px-4 pb-2 pt-4 text-[15px] leading-6 text-foreground placeholder:text-muted-foreground focus:outline-none disabled:opacity-60" rows={1} />
