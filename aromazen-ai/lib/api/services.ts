...
export const api = {
  ...
  documentGenerator: {
    ...
    generate: (accessToken: string, payload: { templateId: string; documentType: 'coa' | 'sds'; fields: Record<string, string>; rows: Record<string, string>[]; fieldLabels?: Record<string, string>; columnLabels?: Record<string, string>; outputFilename?: string; excel?: File | null }) => {
      const form = new FormData(); form.append('template_document_id', payload.templateId); form.append('document_type', payload.documentType); form.append('fields_json', JSON.stringify(payload.fields)); form.append('rows_json', JSON.stringify(payload.rows)); form.append('field_labels_json', JSON.stringify(payload.fieldLabels ?? {})); form.append('column_labels_json', JSON.stringify(payload.columnLabels ?? {})); if (payload.outputFilename?.trim()) form.append('output_filename', payload.outputFilename.trim()); if (payload.excel) form.append('excel_file', payload.excel)
