...
export default function AdminUsersPage() {
  ...
  async function createDepartment(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (!accessToken) return
    const form = new FormData(event.currentTarget); setBusy(true); setError(null)
    try { await api.admin.createDepartment(accessToken, String(form.get('name'))); event.currentTarget.reset(); notify('success', 'Department added successfully.'); await load() } catch (reason) { report(reason, 'Unable to add department.') } finally { setBusy(false) }
  }
...
function DepartmentCard({ department, canManage, onSave, onRemove }: { department: Department; canManage: boolean; onSave: (id: string, name: string) => Promise<void>; onRemove: (id: string) => Promise<void> }) {
  ...
  return <div className="flex gap-2 rounded-lg border border-border bg-card p-3"><input value={name} readOnly={!canManage} onChange={(event) => setName(event.target.value)} className="min-w-0 flex-1 rounded border border-input bg-muted p-2" />{canManage && <><Button variant="outline" size="sm" onClick={() => void onSave(department.id, name)}>Save</Button><Button variant="destructive" size="sm" onClick={() => void onRemove(department.id)}>Remove</Button></>}</div>
}
