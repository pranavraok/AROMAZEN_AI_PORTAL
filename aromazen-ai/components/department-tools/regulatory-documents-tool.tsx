...
export function RegulatoryDocumentsTool() {
  ...
  async function start() {
    ...
    try {
      ...
      notify('success', 'Files processed locally. Review the extracted data or run the optional official database check.')
  ...
  async function enrich() {
    ...
    if (!window.confirm('Check official PubChem and ECHA records? Ingredient names and available CAS/EC numbers will be sent; formula percentages will not be sent. This uses no paid AI.')) return
    setBusy('enrich')
  ...
  return <main className="space-y-5 p-4 md:p-6">
    ...
    {!workflow ? <><section className="grid gap-4 rounded-2xl border border-border bg-card p-4 md:grid-cols-2"><UploadBox title="Regulatory Excel" file={formula} accept=".xlsx,.xlsm" onChange={setFormula} /><UploadBox title="Creation COA" file={coa} accept=".pdf,.docx,.xlsx" onChange={setCoa} /></section><div className="flex justify-end"><Button disabled={!formula || !coa || busy === 'intake'} onClick={() => void start()}>{busy === 'intake' ? <LoaderCircle className="mr-2 h-4 w-4 animate-spin" /> : <FileSpreadsheet className="mr-2 h-4 w-4" />}Process files</Button></div></> : <>
      ...
      <section className="rounded-2xl border border-border bg-card p-4 md:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-semibold">SDS composition and regulatory review</h2><p className="text-xs text-muted-foreground">Source and review information stays in this screen only and is never printed.</p><p className="mt-1 text-[11px] text-muted-foreground">The optional official check sends ingredient names and available CAS/EC numbers to PubChem and ECHA. Formula percentages are not sent.</p></div>{!locked ? <div className="flex gap-2"><Button variant="outline" disabled={busy === 'enrich'} onClick={() => void enrich()}>{busy === 'enrich' ? <LoaderCircle className="mr-2 h-4 w-4 animate-spin" /> : <Search className="mr-2 h-4 w-4" />}{busy === 'enrich' ? 'Checking official databases…' : researchSummary ? 'Refresh official data' : 'Check official data (free)'}</Button><Button variant="outline" disabled={busy === 'enrich'} onClick={() => setRows((current) => [...current, blankIngredient()])}><Plus className="mr-2 h-4 w-4" />Add ingredient</Button></div> : null}</div>
        {busy === 'enrich' ? <div className="mt-4 rounded-xl border border-primary/30 bg-primary/5 p-3 text-sm text-primary"><LoaderCircle className="mr-2 inline h-4 w-4 animate-spin" />Checking PubChem, ECHA, and the approved IFRA/allergen masters. This uses no paid AI.</div> : researchSummary ? <div className={`mt-4 rounded-xl border p-3 text-sm ${researchSummary.failed ? 'border-amber-500/30 bg-amber-500/5 text-amber-300' : 'border-emerald-500/30 bg-emerald-500/5 text-emerald-300'}`}>{researchSummary.mode === 'official' ? <>Official checks completed for {researchSummary.attempted} ingredient{researchSummary.attempted === 1 ? '' : 's'}; {researchSummary.cached} came from the saved master/cache. <strong>No paid AI searches were used.</strong> {researchSummary.unresolved} trade name{researchSummary.unresolved === 1 ? '' : 's'} remain unresolved.{researchSummary.failed ? ` ${researchSummary.failed} source checks can be retried.` : ''}</> : <>One optional AI identity lookup completed. {researchSummary.populated ? 'An official match was found.' : 'No verified match was found.'}</>}</div> : null}
        <div className="mt-4 overflow-auto rounded-xl border border-border"><table className="w-full min-w-[1240px] text-sm"><thead className="bg-muted/60"><tr>{['Ingredient', '% / band', 'CAS', 'EC', 'Classification', 'Review status', 'Sources', 'Action'].map((heading) => <th key={heading} className="p-3 text-left">{heading}</th>)}</tr></thead><tbody>{rows.map((row, index) => <Fragment key={`${row.name}-${index}`}><tr className="border-t border-border">{(['name', 'concentration', 'cas', 'ec', 'classification'] as const).map((key) => <td key={key} className="p-2"><input value={String(row[key] ?? '')} disabled={locked} placeholder="Leave blank if unavailable" onChange={(event) => updateRow(index, key, event.target.value)} className="h-9 w-full rounded-lg border border-border bg-background px-2 disabled:opacity-70" /></td>)}<td className="p-2"><span className={`rounded-full px-2 py-1 text-[10px] ${row.provenance === 'ai_suggested' ? 'bg-amber-500/10 text-amber-400' : row.provenance === 'official_database' || row.provenance === 'approved_master' || row.provenance === 'employee_approved' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-muted text-muted-foreground'}`}>{row.provenance?.replaceAll('_', ' ') ?? 'excel'}</span></td><td className="max-w-52 p-2">{row.sources?.length ? <details><summary className="cursor-pointer text-xs text-primary">{row.sources.length} official source(s)</summary><div className="mt-1 space-y-1">{row.sources.map((url) => <a key={url} href={url} target="_blank" rel="noreferrer" className="block truncate text-[10px] text-primary underline">{url}</a>)}</div></details> : <span className="text-xs text-muted-foreground">None</span>}</td><td className="p-2">{!locked ? <div className="flex items-center gap-2">{!row.cas && !row.ec ? <Button size="sm" variant="outline" disabled={busy === `ai-${index}`} onClick={() => void aiIdentityFallback(index)}>{busy === `ai-${index}` ? <LoaderCircle className="mr-1 h-3 w-3 animate-spin" /> : <Sparkles className="mr-1 h-3 w-3" />}AI fallback</Button> : null}<button type="button" aria-label={`Remove ${row.name || 'ingredient'}`} onClick={() => setRows((current) => current.filter((_, rowIndex) => rowIndex !== index))} className="text-destructive"><Trash2 className="h-4 w-4" /></button></div> : null}</td></tr>
