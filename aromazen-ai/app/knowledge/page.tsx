...
export default function KnowledgePage() {
  ...
  return <AppLayout><div className="space-y-6 p-6">
    ...
    {loading ? <p className="text-sm text-muted-foreground">Loading collections…</p> : <>
      ...
      <Link href={`/knowledge/${collection.slug}`} className="block min-w-0 transition hover:text-primary"><div className="mb-5 flex items-start justify-between"><div className="rounded-xl bg-primary/10 p-2.5 text-primary"><BookOpen className="h-5 w-5" /></div>{collection.is_shared ? <span className="flex items-center gap-1 text-xs text-muted-foreground"><Users className="h-3 w-3" />Shared</span> : <span className="flex items-center gap-1 text-xs text-muted-foreground"><Lock className="h-3 w-3" />Restricted</span>}</div><h2 className="break-words font-semibold text-foreground [overflow-wrap:anywhere]">{collection.name}</h2><div className="mt-4 break-words border-t border-border pt-3 text-xs text-muted-foreground [overflow-wrap:anywhere]">{collection.is_shared ? 'Company-wide' : collection.department_names.join(' · ')} · {collection.document_count} documents</div></Link>
    ...
    {pendingUpload && <div className="fixed inset-0 z-50 grid place-items-center bg-black/65 p-4" role="dialog" aria-modal="true" aria-label="Add Knowledge document"><div className="w-full max-w-lg rounded-2xl border border-border bg-card p-5 shadow-2xl"><h2 className="text-lg font-semibold">Add Knowledge document</h2><p className="mt-1 break-words text-sm text-muted-foreground [overflow-wrap:anywhere]">{pendingUpload.file.name} · {pendingUpload.collectionName}</p><div className="mt-5 grid gap-4 sm:grid-cols-2">
