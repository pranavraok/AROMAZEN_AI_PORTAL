...
export default function KnowledgeAdminPage() {
  ...
  if (!hasPermission('settings.manage')) return <AppLayout><div className="p-6 text-sm text-muted-foreground">Knowledge administration is available to the Owner and Super Admin only.</div></AppLayout>
  return <AppLayout><div className="space-y-6 p-6"><PageHeader title="Manage Knowledge" description="Control collections, access, and every document without developer support." actions={<div className="flex gap-2"><Button variant="outline" onClick={() => router.push('/knowledge')}>Back to Knowledge</Button>{tab === 'collections' && <Button onClick={() => setEditing('new')}><Plus className="mr-1 h-4 w-4" />New collection</Button>}</div>} />
    <div className="flex gap-2 border-b border-border pb-3"><Button variant={tab === 'collections' ? 'default' : 'ghost'} onClick={() => setTab('collections')}>Collections</Button><Button variant={tab === 'documents' ? 'default' : 'ghost'} onClick={() => setTab('documents')}>Documents</Button></div>
    {tab === 'collections' ? <div className="grid gap-4 md:grid-cols-2">{collections.map((collection) => <div key={collection.id} className="rounded-lg border border-border bg-card p-5"><div className="flex items-start justify-between gap-3"><div><h2 className="font-semibold">{collection.name}</h2><p className="mt-1 text-sm text-muted-foreground">{collection.description || 'No description'}</p></div><span className={`rounded-full px-2 py-1 text-xs ${collection.status === 'active' ? 'bg-emerald-500/15 text-emerald-600' : 'bg-muted text-muted-foreground'}`}>{collection.status}</span></div><p className="mt-4 text-xs text-muted-foreground">{collection.is_shared ? 'Company-wide access' : collection.department_names.join(' · ')} · {collection.document_count} documents</p><div className="mt-4 flex gap-2"><Button size="sm" variant="outline" onClick={() => setEditing(collection)}>Edit</Button>{collection.status === 'active' && <Button size="sm" variant="destructive" onClick={() => void archive(collection)}>Archive</Button>}</div></div>)}</div> : <div className="overflow-hidden rounded-lg border border-border bg-card"><table className="w-full text-sm"><thead className="bg-muted/50"><tr><th className="p-4 text-left">Document</th><th className="p-4 text-left">Collection</th><th className="p-4 text-left">Status</th><th className="p-4 text-right">Action</th></tr></thead><tbody>{documents.map((document) => <tr key={document.id} className="border-t border-border"><td className="p-4"><p className="font-medium">{document.name}</p><p className="text-xs text-muted-foreground">{document.extracted_characters.toLocaleString()} characters · v{document.version}</p></td><td className="p-4">{document.collection_name}</td><td className="p-4 capitalize">{document.status}</td><td className="p-4 text-right"><Button size="sm" variant="destructive" onClick={() => void removeDocument(document)}>Remove</Button></td></tr>)}</tbody></table>{documents.length === 0 && <p className="p-6 text-sm text-muted-foreground">No documents uploaded yet.</p>}</div>}
...
function CollectionModal({ collection, departments, busy, onClose, onSubmit }: { collection: AdminKnowledgeCollection | null; departments: Department[]; busy: boolean; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => Promise<void> }) {
  return <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4"><form onSubmit={(event) => void onSubmit(event)} className="w-full max-w-lg rounded-lg border border-border bg-card p-6"><div className="mb-4 flex justify-between"><h2 className="text-lg font-semibold">{collection ? 'Edit collection' : 'New collection'}</h2><Button type="button" variant="ghost" size="sm" onClick={onClose}>Close</Button></div><div className="space-y-3"><input name="name" required defaultValue={collection?.name} placeholder="Collection name" className="w-full rounded border border-input bg-muted p-2" /><textarea name="description" defaultValue={collection?.description ?? ''} placeholder="What belongs in this collection?" className="min-h-20 w-full rounded border border-input bg-muted p-2" /><label className="flex items-center gap-2 text-sm"><input name="is_shared" type="checkbox" defaultChecked={collection?.is_shared} />Company-wide access</label><div className="rounded border border-border p-3"><p className="mb-2 text-sm font-medium">Department access</p>{departments.map((department) => <label key={department.id} className="flex items-center gap-2 py-1 text-sm"><input name="department_ids" type="checkbox" value={department.id} defaultChecked={collection?.department_ids.includes(department.id)} />{department.name}</label>)}</div><Button type="submit" disabled={busy} className="w-full">{busy ? 'Saving…' : 'Save collection'}</Button></div></form></div>
}
