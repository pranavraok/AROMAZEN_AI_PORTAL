...
type TokenUsage = { used_tokens: number; daily_limit: number }
type WorkspaceMessage = ChatMessageDto & { web_sources?: WebSource[]; attachments?: ChatAttachment[]; token_usage?: TokenUsage }
  ...
  async function sendMessage(content: string, attachments: ChatAttachment[] = [], mode: 'chat' | 'image' | 'email' = 'chat', responseMode: ResponseMode = 'quick', options: SendOptions = {}): Promise<boolean> {
    ...
    try {
      ...
      await readEventStream(response, (event, payload) => {
        ...
        if (event === 'token_usage' && payload.used_tokens !== undefined && payload.daily_limit) setMessages((current) => current.map((message) => message.id === assistantId ? { ...message, token_usage: { used_tokens: payload.used_tokens!, daily_limit: payload.daily_limit! } } : message))
