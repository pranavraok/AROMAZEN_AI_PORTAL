...
export function RequireAuthenticatedApp({ children }: { children: React.ReactNode }) {
  ...
  const canUseDocumentGenerator = ['R&D', 'QA & QC'].includes(user.department_name ?? '')
  if (isDocumentGeneratorRoute && !isPlatformAdmin && !canUseDocumentGenerator) return <main className="min-h-screen bg-background grid place-items-center p-6"><div className="max-w-md text-center space-y-3"><h1 className="text-2xl font-semibold text-foreground">Access restricted</h1><p className="text-muted-foreground">This tool is available only to the R&amp;D and QA &amp; QC departments.</p></div></main>
  if (isRndDepartmentTool && !isPlatformAdmin && user.department_name !== 'R&D') return <main className="min-h-screen bg-background grid place-items-center p-6"><div className="max-w-md text-center space-y-3"><h1 className="text-2xl font-semibold text-foreground">Access restricted</h1><p className="text-muted-foreground">This tool is available only to the R&amp;D department.</p></div></main>
  ...
  if (isAccountsGst && !(user.role_names.includes('Admin') || user.department_name === 'Accounts' && user.role_names.some((role) => role === 'Department Admin' || role === 'Employee'))) return <main className="min-h-screen bg-background grid place-items-center p-6"><div className="max-w-md text-center space-y-3"><h1 className="text-2xl font-semibold text-foreground">Access restricted</h1><p className="text-muted-foreground">GST Reconciliation is available to Admin and the Accounts team.</p></div></main>
  return <>{children}</>
}
