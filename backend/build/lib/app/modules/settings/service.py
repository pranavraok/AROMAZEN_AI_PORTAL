...
async def organization_settings(session: AsyncSession, organization_id) -> OrganizationSetting:
    ...
    if value is None:
        ...
        session.add(value)
        await session.flush()
    elif value.default_ai_provider != "auto":
        # Auto is the organization-wide routing policy. Normalize legacy rows so
        # settings refreshes and AI requests cannot fall back to a stale provider.
...
async def provider_runtime_settings(session: AsyncSession, organization_id) -> Settings:
    await organization_settings(session, organization_id)
    return get_settings().model_copy(update={"ai_default_provider": "auto"})
