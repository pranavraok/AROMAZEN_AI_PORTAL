...
export function HrInterviewTool() {
  ...
  return <main className="space-y-5 p-4 md:p-6">
    ...
    <div className={`grid gap-5 ${previewUrl ? 'xl:grid-cols-[minmax(0,1fr)_minmax(440px,.9fr)]' : 'mx-auto w-full max-w-3xl'}`}>
      <div className="space-y-5">
        <section className="rounded-2xl border border-border bg-card p-4 md:p-5">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {FIELD_DEFINITIONS.slice(0, 6).map((field) => <label key={field.key}><span className="mb-1.5 block text-xs text-muted-foreground">{field.label}</span><input type={'type' in field ? field.type : 'text'} value={fields[field.key] ?? ''} placeholder={'placeholder' in field ? field.placeholder : undefined} onChange={(event) => changeField(field.key, event.target.value)} className="h-11 w-full rounded-xl border border-border bg-background px-3 text-sm" /></label>)}
            <details className="md:col-span-2 rounded-xl border border-border"><summary className="cursor-pointer p-3 text-sm font-medium">Additional candidate details</summary><div className="grid gap-4 border-t border-border p-3 md:grid-cols-2">{FIELD_DEFINITIONS.slice(6).map((field) => <label key={field.key}><span className="mb-1.5 block text-xs text-muted-foreground">{field.label}</span><input type={'type' in field ? field.type : 'text'} value={fields[field.key] ?? ''} placeholder={'placeholder' in field ? field.placeholder : undefined} onChange={(event) => changeField(field.key, event.target.value)} className="h-11 w-full rounded-xl border border-border bg-background px-3 text-sm" /></label>)}</div></details>
          </div>
        </section>
