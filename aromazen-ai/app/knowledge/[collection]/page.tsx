...
import { ChevronLeft, Download, ExternalLink, FileText, Folder, Lock, Shield, Users } from 'lucide-react'
    ...
    {loading ? <p className="text-sm text-muted-foreground">Loading collections…</p> : <>
      ...
      <div className="overflow-hidden rounded-lg border border-border bg-card">
        ...
        {filteredDocs.length === 0 ? <p className="p-6 text-sm text-muted-foreground">No documents found for this filter.</p> : <div className="divide-y divide-border">
          {filteredDocs.map((item) => <div key={item.document.id} className="flex items-center gap-3 p-4">
            <FileText className="h-5 w-5 shrink-0 text-primary" />
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-medium">{item.document.name}</p>
              <p className="mt-1 text-xs text-muted-foreground">
                {collectionFilter === null && <><span className="font-medium text-foreground/70">{item.collection.name}</span> · </>}{folderLabel(item.document)} · {formatSize(item.document.size_bytes)} · v{item.document.version} · {item.document.extracted_characters.toLocaleString()} chars
            ...
            <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${item.document.status === 'ready' ? 'bg-emerald-500/15 text-emerald-600' : item.document.status === 'failed' ? 'bg-destructive/15 text-destructive' : 'bg-amber-500/15 text-amber-700'}`}>{item.document.status === 'ready' ? 'Ready' : item.document.status === 'failed' ? 'Failed' : item.document.status === 'uploaded' ? 'Needs processing' : 'Processing'}</span>
            ...
            {item.document.status !== 'ready' && hasPermission('knowledge.write') && <Button variant="outline" size="sm" onClick={() => void processDocument(item)}>Process now</Button>}
