...
export default function CashFlowPage() {
  ...
  return <AppLayout><main className="mx-auto w-full max-w-6xl space-y-5 p-5 md:p-6">
    <PageHeader title="Monthly Cash Flow" description="Generate, download and securely archive the protected monthly PDF." actions={<div className="flex flex-wrap gap-2"><Button variant="outline" disabled={busy !== null} onClick={() => void template('cash')}><Download className="mr-1 h-4 w-4" />Cash-flow template</Button><Button variant="outline" disabled={busy !== null} onClick={() => void template('assets')}><Download className="mr-1 h-4 w-4" />Asset template</Button></div>} />
