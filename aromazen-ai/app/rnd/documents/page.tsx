  ...
  return <AppLayout><div className="space-y-5 p-4 md:p-6"><PageHeader title="Smart COA/SDS Creation" description="Choose a document, add the available facts, review and download." />
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
      <section className="space-y-6 rounded-lg border border-border bg-card p-5">
        <div><label className="mb-2 block text-sm font-medium">Word template</label><select value={templateId} onChange={(event) => setTemplateId(event.target.value)} className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"><option value="">Select a template</option>{templates.map((item) => <option key={item.id} value={item.id}>{item.name} ({item.document_type.toUpperCase()})</option>)}</select>{selected && <InfoTip label="Template source">From {selected.collection_name}. The original file will not be changed.</InfoTip>}</div>
        ...
        {schema && <>
          ...
          <summary className="flex cursor-pointer list-none items-center justify-between p-4"><div className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-primary" /><div><div className="flex items-center gap-1"><h2 className="text-sm font-semibold">Smart fill from voice or notes</h2><InfoTip label="About smart fill">Optional — describe the document naturally and let AI organize it.</InfoTip></div></div></div><ChevronRight className="h-4 w-4 transition group-open:rotate-90" /></summary>
          <div className="space-y-3 border-t border-primary/20 p-4">
            <div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-primary" /><h2 className="font-semibold">AI Draft Assistant</h2></div><InfoTip label="Voice entry help">Speak naturally, pause, and correct yourself whenever needed. Your words appear below; when you press Done, the complete recording is professionally rechecked before any field is filled.</InfoTip></div><div className="flex items-center gap-2"><VoiceInputButton continuous stopSignal={voiceStopSignal} label="Start speaking" onTranscript={acceptSpeech} onInterim={setLiveSpeech} onListeningChange={handleListeningChange} onRecordingReady={(file) => void processCompleteRecording(file)} /><span className={`text-xs font-medium ${isListening ? 'text-red-400' : 'text-muted-foreground'}`}>{isListening ? 'Listening…' : 'Start speaking'}</span></div></div>
          ...
          {step !== steps.length - 1 && step !== 1 && <div><div className="flex items-center gap-1"><h2 className="font-semibold">{schema.document_type === 'coa' ? 'Product and batch details' : step === 0 ? 'Product and company details' : step === 2 ? 'Safety classification' : 'Physical properties'}</h2><InfoTip label="About document fields">Only this section is shown. Required information is marked clearly.</InfoTip></div></div>}
          ...
          {step === 1 && <div className="space-y-3 border-t border-border pt-5"><div className="flex items-center justify-between"><div><div className="flex items-center gap-1"><h2 className="font-semibold">{schema.document_type === 'coa' ? 'COA Test Results' : 'Ingredients'}</h2><InfoTip label="Table entry help">{schema.document_type === 'coa' ? 'Approved parameters stay fixed. Enter only Specification and Result.' : 'Add each ingredient once. Unknown optional identifiers can remain blank.'}</InfoTip></div></div>{schema.document_type === 'sds' && <Button type="button" variant="outline" onClick={addRow}><Plus className="mr-2 h-4 w-4" />Add ingredient</Button>}</div>
      ...
      <aside className="space-y-4">
        {canUploadTemplate && <div className="rounded-lg border border-primary/30 bg-card p-5"><div className="flex items-center gap-1"><h2 className="font-semibold">Upload Word Template</h2><InfoTip label="Word template help">Upload your approved SDS or COA format as a DOCX file. Your template is private to your account, and a later upload of the same type replaces your previous one.</InfoTip></div><select value={templateUploadType} onChange={(event) => setTemplateUploadType(event.target.value as 'coa' | 'sds')} className="mt-4 w-full rounded-md border border-border bg-background px-3 py-2 text-sm"><option value="sds">Safety Data Sheet (SDS)</option><option value="coa">Certificate of Analysis (COA)</option></select><label className="mt-3 flex cursor-pointer items-center justify-center rounded-md border border-dashed border-border p-5 text-center text-sm hover:bg-muted"><input type="file" accept=".docx,application/vnd.openxmlformats-officedocument.wordprocessingml.document" className="hidden" onChange={(event: ChangeEvent<HTMLInputElement>) => setTemplateFile(event.target.files?.[0] ?? null)} /><span><Upload className="mx-auto mb-2 h-5 w-5" />{templateFile?.name ?? 'Choose DOCX template'}</span></label><Button type="button" className="mt-3 w-full" disabled={!templateFile || uploadingTemplate} onClick={() => void uploadTemplate()}><Upload className="mr-2 h-4 w-4" />{uploadingTemplate ? 'Uploading template…' : `Upload ${templateUploadType.toUpperCase()} Template`}</Button></div>}
        {schema?.can_edit_filename && <div className="rounded-lg border border-border bg-card p-5"><label className="text-sm font-semibold">Document filename</label><InfoTip label="Filename permissions">Available to the Super Admin and administrators.</InfoTip><div className="mt-3 flex items-center gap-2"><input value={outputFilename} onChange={(event) => setOutputFilename(event.target.value)} placeholder="Example: Orange Blossom COA" className="min-w-0 flex-1 rounded-md border border-border bg-background px-3 py-2 text-sm" /><span className="text-xs text-muted-foreground">.docx</span></div></div>}
        {schema?.document_type === 'coa' && <div className="rounded-lg border border-border bg-card p-5"><div className="flex items-center gap-1"><h2 className="font-semibold">Excel entry</h2><InfoTip label="Excel entry help">Optional shortcut for COA test results.</InfoTip></div><Button variant="outline" onClick={() => void downloadExcel()} disabled={!templateId} className="mt-4 w-full"><Download className="mr-2 h-4 w-4" />Download Excel format</Button><label className="mt-3 flex cursor-pointer items-center justify-center rounded-md border border-dashed border-border p-5 text-center text-sm hover:bg-muted"><input type="file" accept=".xlsx" className="hidden" onChange={(event: ChangeEvent<HTMLInputElement>) => setExcel(event.target.files?.[0] ?? null)} /><span><Upload className="mx-auto mb-2 h-5 w-5" />{excel?.name ?? 'Choose filled XLSX file'}</span></label></div>}
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-4 text-sm"><p className="font-medium">Review before issue</p><InfoTip label="About document review">The AI only organizes stated facts. Missing information remains blank, and SDS regulatory information still requires qualified review.</InfoTip></div>
