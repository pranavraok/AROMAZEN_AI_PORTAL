...
export default function KnowledgeAdminPage() {
  ...
  return <AppLayout><div className="space-y-6 p-4 sm:p-6"><PageHeader title="Manage Knowledge" description="Control collections, access, and every document without developer support." actions={<div className="flex gap-2"><Button variant="outline" onClick={() => router.push('/knowledge')}>Back to Knowledge</Button>{tab === 'collections' && <Button onClick={() => setEditing('new')}><Plus className="mr-1 h-4 w-4" />New collection</Button>}</div>} />
    ...
    {tab === 'collections' ? <div className="grid gap-4 md:grid-cols-2">{collections.map((collection) => <div key={collection.id} className="rounded-lg border border-border bg-card p-4 sm:p-5"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h2 className="break-words font-semibold">{collection.name}</h2><p className="mt-1 break-words text-sm text-muted-foreground">{collection.description || 'No description'}</p></div><span className={`shrink-0 rounded-full px-2 py-1 text-xs ${collection.status === 'active' ? 'bg-emerald-500/15 text-emerald-600' : 'bg-muted text-muted-foreground'}`}>{collection.status}</span></div><p className="mt-4 break-words text-xs text-muted-foreground">{collection.is_shared ? 'Company-wide access' : collection.department_names.join(' · ')} · {collection.document_count} documents</p><div className="mt-4 flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => setEditing(collection)}>Edit</Button>{collection.status === 'active' && <Button size="sm" variant="secondary" onClick={() => void archive(collection)}>Archive</Button>}<Button size="sm" variant="destructive" onClick={() => void removeCollection(collection)}><Trash2 className="mr-1 h-3 w-3" />Delete</Button></div></div>)}</div> : <div className="flex min-h-[60vh] min-w-0 flex-col gap-3 md:flex-row md:gap-4">
      {/* Left: Collection list */}
      <div className="flex shrink-0 gap-1 overflow-x-auto rounded-lg border border-border bg-card p-2 md:w-64 md:flex-col md:overflow-x-hidden md:overflow-y-auto md:p-3">
        <button onClick={() => setSelectedCollectionId(null)} className={`min-w-max rounded-lg px-3 py-2.5 text-left text-sm font-medium transition-colors md:w-full md:min-w-0 ${selectedCollectionId === null ? 'bg-primary/10 text-primary' : 'hover:bg-muted text-foreground'}`}>
          All documents
        ...
        {collections.filter((c) => docCountByCollection[c.id]).map((collection) => (
          <button key={collection.id} onClick={() => setSelectedCollectionId(collection.id)} className={`min-w-max rounded-lg px-3 py-2.5 text-left text-sm font-medium transition-colors md:w-full md:min-w-0 ${selectedCollectionId === collection.id ? 'bg-primary/10 text-primary' : 'hover:bg-muted text-foreground'}`}>
            {collection.name}
      ...
      <div className="min-w-0 flex-1 overflow-hidden rounded-lg border border-border bg-card">
        ...
        <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[640px] text-sm"><thead className="bg-muted/50"><tr><th className="p-4 text-left">Document</th><th className="p-4 text-left">Collection</th><th className="p-4 text-left">Status</th><th className="p-4 text-right">Action</th></tr></thead><tbody>{filteredDocuments.map((document) => <tr key={document.id} className="border-t border-border"><td className="p-4"><p className="font-medium">{document.name}</p><p className="text-xs text-muted-foreground">{document.extracted_characters.toLocaleString()} characters · v{document.version}</p></td><td className="p-4">{document.collection_name}</td><td className="p-4 capitalize">{document.status}</td><td className="p-4 text-right"><Button size="sm" variant="destructive" onClick={() => void removeDocument(document)}>Remove</Button></td></tr>)}</tbody></table></div>
        <div className="divide-y divide-border md:hidden">{filteredDocuments.map((document) => <article key={document.id} className="space-y-3 p-4"><div className="min-w-0"><p className="break-words font-medium">{document.name}</p><p className="mt-1 text-xs text-muted-foreground">{document.collection_name} · {document.extracted_characters.toLocaleString()} characters · v{document.version}</p></div><div className="flex items-center justify-between gap-3"><span className="capitalize text-xs text-muted-foreground">{document.status}</span><Button size="sm" variant="destructive" onClick={() => void removeDocument(document)}>Remove</Button></div></article>)}</div>
