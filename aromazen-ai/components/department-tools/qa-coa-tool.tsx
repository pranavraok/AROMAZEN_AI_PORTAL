...
import { Check, Download, ExternalLink, Eye, FileText, LoaderCircle, Printer, Sparkles, Upload } from 'lucide-react'
...
export function QaCoaTool() {
  ...
  const completedFields = useMemo(() => schema?.fields.filter((field) => fields[field.key]?.trim()).length ?? 0, [fields, schema])
  const requiredMissing = useMemo(() => schema?.fields.filter((field) => field.required && !fields[field.key]?.trim()) ?? [], [fields, schema])
  ...
  async function organizeNotes(notes: string) {
    ...
    setDrafting(true)
    try {
      const result = await api.documentGenerator.draftFromNotes(accessToken, { templateId: template.id, notes, currentFields: fields, currentRows: rows })
  ...
  async function generatePreview() {
    ...
    setBusy(true)
    try {
      const result = await api.documentGenerator.generate(accessToken, { templateId: template.id, documentType: 'coa', fields, rows })
      setGenerated(result)
  ...
  return <main className="space-y-5 p-4 md:p-6"><PageHeader title="QA · Certificate of Analysis" description="Prepare, review, print and download the approved COA. Use voice or type the details manually." />
    ...
    <section className="flex flex-col gap-4 rounded-2xl border border-primary/25 bg-primary/5 p-4 md:flex-row md:items-center md:justify-between"><div className="flex min-w-0 items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-primary/15 text-primary"><FileText className="h-5 w-5" /></span><div className="min-w-0"><h2 className="font-semibold">Current COA master</h2><p className="truncate text-sm text-muted-foreground">{template.name} · Version {template.version ?? 1} · Quality Assurance Knowledge</p><p className="mt-1 text-xs text-muted-foreground">The approved product fields and six test parameters are mapped automatically. Uploading a new DOCX replaces this master for the whole QA department.</p></div></div><div className="flex shrink-0 flex-wrap gap-2"><Button variant="outline" onClick={() => void viewTemplate()}><Eye className="mr-2 h-4 w-4" />View template</Button><Button variant="outline" onClick={() => window.open(canvaUrl, '_blank', 'noopener,noreferrer')}><ExternalLink className="mr-2 h-4 w-4" />Edit in Canva</Button>{hasPermission('knowledge.write') && <><input ref={uploadRef} hidden type="file" accept=".docx,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={(event) => void replaceMaster(event.target.files?.[0] ?? null)} /><Button disabled={replacing} onClick={() => uploadRef.current?.click()}>{replacing ? <LoaderCircle className="mr-2 h-4 w-4 animate-spin" /> : <Upload className="mr-2 h-4 w-4" />}{replacing ? 'Mapping fields' : 'Upload master template'}</Button></>}</div></section>
    <section className="rounded-2xl border border-primary/25 bg-primary/5 p-4 md:p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-primary" /><h2 className="font-semibold">Smart fill from voice or notes</h2></div><p className="mt-1 max-w-3xl text-sm text-muted-foreground">Speak naturally and correct yourself when needed. Press Done when finished; the complete recording is rechecked and mapped into the approved fields.</p></div><div className="flex items-center gap-2"><VoiceInputButton continuous stopSignal={voiceStopSignal} label="Start speaking" onTranscript={acceptSpeech} onInterim={setLiveSpeech} onListeningChange={handleListeningChange} onRecordingReady={(file) => void processCompleteRecording(file)} /><span className={`text-xs font-medium ${isListening ? 'text-red-400' : 'text-muted-foreground'}`}>{isListening ? 'Listening…' : 'Start speaking'}</span></div></div><textarea rows={5} value={draftNotes} onChange={(event) => { notesRef.current = event.target.value; setDraftNotes(event.target.value) }} placeholder="Example: Product Rose Absolute, code RA 101, batch B25, manufacturing date 3 September 2026, appearance result passes…" className="mt-4 w-full rounded-xl border border-border bg-background p-3 text-sm" />{liveSpeech && <div className="mt-3 rounded-xl border border-emerald-500/30 bg-emerald-500/5 px-3 py-2 text-sm text-emerald-300"><span className="mr-2 text-xs font-semibold uppercase tracking-wide">Hearing</span>{liveSpeech}</div>}<div className="mt-3 flex flex-wrap items-center justify-between gap-3"><span className="text-xs text-muted-foreground">{drafting ? 'Formatting and filling the COA…' : isListening ? 'Keep speaking. Press Done only when finished.' : 'Review the transcript, then fill the draft.'}</span><Button disabled={drafting || (!draftNotes.trim() && !isListening)} onClick={finishAndFillDraft}><Sparkles className="mr-2 h-4 w-4" />{drafting ? 'Filling draft…' : 'Done — Fill Draft'}</Button></div>{unassignedNotes && <p className="mt-3 rounded-xl border border-amber-500/30 bg-amber-500/5 p-3 text-xs text-amber-400">Not placed automatically: {unassignedNotes}</p>}</section>
    <div className={`grid gap-5 ${previewUrl ? 'xl:grid-cols-[minmax(0,1fr)_minmax(440px,.9fr)]' : 'mx-auto w-full max-w-4xl'}`}><div className="space-y-5"><section className="rounded-2xl border border-border bg-card p-4 md:p-5"><div className="mb-5"><p className="text-xs font-medium uppercase tracking-[.14em] text-primary">Step 1 · Enter details</p><h2 className="mt-1 text-lg font-semibold">Product and batch details</h2><p className="mt-1 text-xs text-muted-foreground">{completedFields} of {schema.fields.length} fields completed. Required information is marked.</p></div><div className="grid gap-4 md:grid-cols-2">{schema.fields.map((field) => <label key={field.key}><span className="mb-1.5 block text-xs text-muted-foreground">{field.label}{field.required ? ' *' : ''}</span><input value={fields[field.key] ?? ''} onChange={(event) => changeField(field.key, event.target.value)} className="h-11 w-full rounded-xl border border-border bg-background px-3 text-sm" /></label>)}</div></section>
      <section className="rounded-2xl border border-border bg-card p-4 md:p-5"><div><p className="text-xs font-medium uppercase tracking-[.14em] text-primary">Step 2 · Test results</p><h2 className="mt-1 text-lg font-semibold">Approved COA parameters</h2><p className="mt-1 text-xs text-muted-foreground">Parameter names come from the active master. Enter only Specification and Result.</p></div><div className="mt-4 overflow-x-auto rounded-xl border border-border"><table className="w-full min-w-[42rem] text-sm"><thead className="bg-muted/70"><tr><th className="w-1/3 px-4 py-3 text-left">Parameter</th><th className="w-1/3 px-4 py-3 text-left">Specification</th><th className="w-1/3 px-4 py-3 text-left">Result</th></tr></thead><tbody>{rows.map((row, index) => <tr key={`${row.parameter}-${index}`} className="border-t border-border"><td className="bg-muted/30 px-4 py-3 font-medium">{row.parameter}</td><td className="p-2"><input aria-label={`${row.parameter} specification`} value={row.specification ?? ''} onChange={(event) => updateRow(index, 'specification', event.target.value)} placeholder="Enter specification" className="h-10 w-full rounded-lg border border-border bg-background px-3" /></td><td className="p-2"><input aria-label={`${row.parameter} result`} value={row.result ?? ''} onChange={(event) => updateRow(index, 'result', event.target.value)} placeholder="Enter result" className="h-10 w-full rounded-lg border border-border bg-background px-3" /></td></tr>)}</tbody></table></div></section>
