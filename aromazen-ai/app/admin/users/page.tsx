...
export default function AdminUsersPage() {
  ...
  async function invite(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setInvitation(result); setInviteMessage(defaultInvitationMessage(result.user.full_name)); setInviteCc(''); setInvitationEmailSent(false); notify('success', 'Invitation created. Review and send the email.'); await load()
  ...
  return <AppLayout><div className="space-y-6 p-6">
    <PageHeader title="Administration" description="Manage people, departments, access, and accountability" actions={<Button onClick={() => { setIsInviteOpen(true); setInvitation(null); setInvitationEmailSent(false); setInviteCc(''); setInviteSubject('Invitation to join AROMAZEN AI'); setInviteMessage('') }} className="bg-primary"><Plus className="mr-1 w-4 h-4" />Invite user</Button>} />
    ...
    {isInviteOpen && <Modal title="Invite employee" onClose={() => setIsInviteOpen(false)}><>{invitation ? invitationEmailSent ? <div className="space-y-4 text-center"><span className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-emerald-500/10 text-emerald-400"><CheckCircle2 className="h-6 w-6" /></span><div><p className="font-semibold">Invitation email sent</p><p className="mt-1 text-sm text-muted-foreground">Sent to {invitation.user.email} through the HR mailbox.</p></div><Button className="w-full" onClick={() => setIsInviteOpen(false)}>Done</Button></div> : <form onSubmit={sendInvitationEmail} className="space-y-4"><div className="rounded-xl border border-border bg-muted/30 p-3"><div className="flex items-center gap-2 text-sm font-medium"><Mail className="h-4 w-4 text-primary" />Email invitation</div><p className="mt-1 text-xs text-muted-foreground">The current HR signature and logo will be added automatically.</p></div><label className="block"><span className="mb-1.5 block text-xs text-muted-foreground">To</span><input readOnly value={invitation.user.email} className="w-full rounded border border-input bg-muted p-2 text-sm" /></label><label className="block"><span className="mb-1.5 block text-xs text-muted-foreground">CC <span className="font-normal">(optional)</span></span><input type="text" inputMode="email" value={inviteCc} onChange={(event) => setInviteCc(event.target.value)} placeholder="Separate multiple emails with commas" className="w-full rounded border border-input bg-muted p-2 text-sm" /></label><label className="block"><span className="mb-1.5 block text-xs text-muted-foreground">Subject</span><input required value={inviteSubject} onChange={(event) => setInviteSubject(event.target.value)} className="w-full rounded border border-input bg-muted p-2 text-sm" /></label><label className="block"><span className="mb-1.5 block text-xs text-muted-foreground">Message</span><textarea required rows={10} value={inviteMessage} onChange={(event) => setInviteMessage(event.target.value)} className="w-full resize-y rounded border border-input bg-muted p-2 text-sm leading-6" /><span className="mt-1 block text-[11px] text-muted-foreground">Keep {'{{invitation_link}}'} where the secure activation link should appear.</span></label><p className="text-xs text-muted-foreground">Valid until {new Date(invitation.expires_at).toLocaleString()}.</p><Button type="submit" disabled={busy || !inviteSubject.trim() || !inviteMessage.trim()} className="w-full"><Send className="mr-2 h-4 w-4" />{busy ? 'Sending...' : 'Send via Email'}</Button></form> : <form onSubmit={invite} className="space-y-3"><input name="full_name" required placeholder="Full name" className="w-full rounded border border-input bg-muted p-2" /><input name="email" type="email" required placeholder="Work or shared email" className="w-full rounded border border-input bg-muted p-2" /><input name="phone_number" type="tel" placeholder="Unique phone (optional unless email is shared)" className="w-full rounded border border-input bg-muted p-2" /><select name="department_id" className="w-full rounded border border-input bg-muted p-2"><option value="">No department</option>{departments.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select name="role_id" required className="w-full rounded border border-input bg-muted p-2"><option value="">Choose role</option>{roles.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><Button type="submit" disabled={busy} className="w-full">{busy ? 'Creating...' : 'Create invitation'}</Button></form>}</></Modal>}
