...
import { QaPhonePairing } from '@/components/department-tools/qa-phone-pairing'
...
export function QaCoaTool() {
  ...
  const organizeNotes = useCallback(async (notes: string) => {
  ...
  }, [accessToken, fieldLabels, fields, notify, rows, template])

  const acceptPhoneTranscript = useCallback((text: string) => {
    notesRef.current = text
    setDraftNotes(text)
    setLiveSpeech('')
  }, [])

  const completePhoneTranscript = useCallback(async (text: string) => {
    acceptPhoneTranscript(text)
    await organizeNotes(text)
  }, [acceptPhoneTranscript, organizeNotes])
  ...
  async function viewTemplate() {
    ...
    const preview = beginBlobPreview(template.name)
    try {
      const file = await api.documentGenerator.templateContent(accessToken, template.id)
      showBlobPreview(preview, file.blob, template.name)
  ...
  return <main className="qa-coa-page min-w-0 space-y-5 p-3 sm:p-4 md:p-6"><PageHeader title="QA · Certificate of Analysis" description="Prepare, review, print and download the approved COA. Use voice or type the details manually." />
