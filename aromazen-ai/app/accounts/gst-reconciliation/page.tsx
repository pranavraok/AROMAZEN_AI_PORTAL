...
const statusClasses: Record<GstReconciliationStatus, string> = { matched: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400', mismatch: 'bg-red-500/10 text-red-600 dark:text-red-400', books_only: 'bg-amber-500/10 text-amber-600 dark:text-amber-400', portal_only: 'bg-blue-500/10 text-blue-600 dark:text-blue-400', incomplete_books: 'bg-orange-500/10 text-orange-600 dark:text-orange-400', duplicate: 'bg-purple-500/10 text-purple-600 dark:text-purple-400' }
...
function FileInput({ label, hint, file, onChange }: { label: string; hint: string; file: File | null; onChange: (file: File | null) => void }) {
  return <label className="group flex min-h-32 cursor-pointer flex-col justify-between rounded-2xl border border-dashed border-border bg-card p-4 transition hover:border-primary/50 hover:bg-primary/[0.02]">
    ...
    <span className={`mt-3 truncate rounded-lg px-3 py-2 text-xs ${file ? 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-300' : 'bg-muted text-muted-foreground'}`}>{file?.name ?? 'Choose Excel file'}</span>
  </label>
}
...
export default function GstReconciliationPage() {
  ...
  return <AppLayout><div className="mx-auto max-w-[1500px] space-y-6 p-6">
    ...
    {result ? <>
      <section className="rounded-2xl border border-border bg-card p-5"><div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-wider text-primary">{result.period}</p><h2 className="mt-1 text-xl font-semibold">{result.company_name || 'GST reconciliation result'}</h2><p className="mt-1 text-xs text-muted-foreground">GSTIN {result.company_gstin || '—'}</p></div><div className="flex items-center gap-2 text-sm text-emerald-600 dark:text-emerald-400"><CheckCircle2 className="h-4 w-4" />Check completed</div></div></section>
      <section className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">{[
        ['Matched', result.summary.matched, 'text-emerald-600 dark:text-emerald-400'], ['Mismatch', result.summary.mismatched, 'text-red-600 dark:text-red-400'], ['Tally only', result.summary.books_only, 'text-amber-600 dark:text-amber-400'], ['Portal only', result.summary.portal_only, 'text-blue-600 dark:text-blue-400'], ['Incomplete', result.summary.incomplete_books, 'text-orange-600 dark:text-orange-400'], ['Duplicates', result.summary.duplicates, 'text-purple-600 dark:text-purple-400'],
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><div className="rounded-xl bg-muted/40 p-4"><p className="text-xs text-muted-foreground">Complete Tally invoice value</p><p className="mt-1 font-semibold">{money(result.summary.book_invoice_value)}</p>{result.summary.incomplete_books > 0 ? <p className="mt-1 text-xs text-muted-foreground">Excludes incomplete Tally rows shown below.</p> : null}</div><div className="rounded-xl bg-muted/40 p-4"><p className="text-xs text-muted-foreground">Portal invoice value</p><p className="mt-1 font-semibold">{money(result.summary.portal_invoice_value)}</p></div><div className="rounded-xl bg-muted/40 p-4"><p className="text-xs text-muted-foreground">Portal credit/debit notes</p><p className="mt-1 font-semibold">{result.summary.portal_credit_notes}</p></div><div className="rounded-xl bg-muted/40 p-4"><p className="text-xs text-muted-foreground">Portal import records</p><p className="mt-1 font-semibold">{result.summary.portal_imports}</p></div></section>
      {result.warnings.length ? <div className="rounded-xl border border-amber-500/25 bg-amber-500/10 p-4 text-xs text-amber-800 dark:text-amber-200"><ul className="space-y-1">{result.warnings.map((warning) => <li key={warning}>• {warning}</li>)}</ul></div> : null}
