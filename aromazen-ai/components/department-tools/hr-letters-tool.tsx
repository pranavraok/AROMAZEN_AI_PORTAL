...
import { Check, Copy, Download, ExternalLink, Eye, FileText, Languages, LoaderCircle, Mail, Plus, Printer, Send, Sparkles, Trash2, Undo2, Upload, X } from 'lucide-react'
...
type AddedSalaryRow = { key: string; label: string }
...
export function HrLettersTool() {
  ...
  const [newSalaryRowLabel, setNewSalaryRowLabel] = useState('')
  const [addedSalaryRows, setAddedSalaryRows] = useState<AddedSalaryRow[]>([])
  const [deletedSalaryRowKeys, setDeletedSalaryRowKeys] = useState<string[]>([])
  const customSalaryRowId = useRef(0)
  ...
  const annexureRows = useMemo(() => {
    const base = (template?.salary_rows ?? []).filter((row) => !deletedSalaryRowKeys.includes(row.key))
    return [...base, ...addedSalaryRows.map((row) => ({ key: row.key, label: row.label, columns: salaryColumns.map((column) => column.key) }))]
  }, [addedSalaryRows, deletedSalaryRowKeys, salaryColumns, template?.salary_rows])
  const payloadFields = useMemo(() => ({ ...fields, ...(usesUnitAddress ? { signatory_name: OFFER_SIGNERS.find((signer) => signer.key === selectedSigner)?.name ?? 'Swathi Nayak' } : {}), ...Object.fromEntries(annexureRows.flatMap((row) => salaryColumns.map((column) => [
    `salary_${row.key}_${column.key}`, resolvedSalaryValues[`${activeScope}:${row.key}:${column.key}`] ?? '',
  ]))) }), [activeScope, annexureRows, fields, resolvedSalaryValues, salaryColumns, selectedSigner, usesUnitAddress])
  ...
  function addSalaryRow(label: string) {
    const trimmed = label.trim()
    if (!trimmed) return
    if (addedSalaryRows.length >= 12) { notify('error', 'Up to 12 extra rows can be added.'); return }
    customSalaryRowId.current += 1
    const key = `custom_${customSalaryRowId.current.toString(36)}`
    setAddedSalaryRows((current) => [...current, { key, label: trimmed }])
    clearPreview()
  }
  function deleteSalaryRow(key: string) {
    setAddedSalaryRows((current) => current.filter((row) => row.key !== key))
    setDeletedSalaryRowKeys((current) => current.includes(key) ? current : [...current, key])
    setSalaryValues((current) => Object.fromEntries(Object.entries(current).filter(([entryKey]) => !entryKey.startsWith(`${activeScope}:${key}:`))))
    clearPreview()
  }
  function restoreSalaryRow(key: string) {
    setDeletedSalaryRowKeys((current) => current.filter((entry) => entry !== key))
    clearPreview()
  }
  ...
  function choose(key: string) { setTemplateKey(key); setAddedSalaryRows([]); setDeletedSalaryRowKeys([]); setPreviewUrl((current) => { if (current) URL.revokeObjectURL(current); return null }) }
  ...
  async function generatePreview() { if (!accessToken || !template) return; const missing = visibleFields.filter((field) => field.required && !fields[field.key]?.trim()).map((field) => field.label); if (missing.length > 0) { notify('error', `Complete the required fields: ${missing.join(', ')}.`); return } setBusy(true); try { let response: Response | null = null; for (let attempt = 0; attempt < 2; attempt++) { try { response = await fetch('/api/v1/hr-letters/preview', { method: 'POST', headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ template_key: template.key, unit_number: selectedUnit, signer_key: selectedSigner, fields: payloadFields, deleted_salary_rows: deletedSalaryRowKeys, added_salary_rows: addedSalaryRows }) }); if (response.ok || response.status < 500) break } catch { if (attempt === 1) throw new Error('The server is unreachable. If it was just restarted, try again in a moment.') } if (attempt === 0) await new Promise((resolve) => setTimeout(resolve, 1500)) } if (!response) throw new Error('Unable to generate this letter.'); if (!response.ok) { const error = await response.json().catch(() => null); throw new Error(error?.detail ?? `Unable to generate this letter (server responded ${response.status}. If the server was just restarted, try again.)`) } const url = URL.createObjectURL(await response.blob()); setPreviewUrl((current) => { if (current) URL.revokeObjectURL(current); return url }); setEmail((current) => ({ ...current, subject: current.subject || `${template.title} - ${fields.employee_name || 'Employee'}`, message: current.message || `Dear ${fields.employee_name || 'Employee'},\n\nPlease find your ${template.title.toLowerCase()} attached.` })); } catch (error) { notify('error', error instanceof Error ? error.message : 'Unable to generate this letter.') } finally { setBusy(false) } }
  ...
  async function sendEmail() { if (!accessToken || !template || !email.recipient.trim()) return; setSending(true); try { const response = await fetch('/api/v1/hr-letters/send', { method: 'POST', headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ template_key: template.key, unit_number: selectedUnit, signer_key: selectedSigner, fields: payloadFields, deleted_salary_rows: deletedSalaryRowKeys, added_salary_rows: addedSalaryRows, recipient_email: email.recipient, cc_emails: parseEmailList(email.cc), subject: email.subject, message: email.message }) }); if (!response.ok) { const error = await response.json().catch(() => null); throw new Error(error?.detail ?? `Unable to send this letter (server responded ${response.status}. If the server was just restarted, try again.)`) } notify('success', `${usesUnitAddress ? `Unit ${selectedUnit} ` : ''}${template.title} sent successfully through HR Zoho Mail.`); setShowEmail(false) } catch (error) { notify('error', error instanceof Error ? error.message : 'Unable to send this letter.') } finally { setSending(false) } }
  ...
  return <main className="space-y-4 p-4 md:p-6"><PageHeader title="HR Letters" actions={<InfoTip label="About HR Letters" align="right">Offer Letters use the selected unit address, digital signature and company seal. The other letters are prepared with space for official letterhead printing.</InfoTip>} />
    ...
    <div className={`grid w-full gap-5 ${previewUrl ? 'xl:grid-cols-[minmax(0,1fr)_minmax(440px,.9fr)]' : 'mx-auto max-w-3xl'}`}><div className="space-y-4"><section className="rounded-2xl border border-border bg-card p-4 md:p-5"><div className="mb-4 flex items-center gap-2"><div className="flex items-center gap-1"><h2 className="text-lg font-semibold">{template.title}</h2><InfoTip label="About these fields">Only the mapped fields below change; approved wording in the master remains fixed.{(activeKey === 'spot_appreciation' || activeKey === 'special_increment') && <><br />For descriptive fields, enter keywords and select AI draft to create one short editable sentence.</>}</InfoTip></div></div><div className="grid gap-4 md:grid-cols-2"><LetterFields items={visibleFields.slice(0, 8)} values={fields} onChange={change} canGenerate={canGenerateField} generatingKey={generatingField} onGenerate={(field) => void generateField(field)} />{visibleFields.length > 8 && <details className="md:col-span-2 rounded-xl border border-border"><summary className="cursor-pointer p-3 text-sm font-medium">More fields <span className="ml-1 text-xs font-normal text-muted-foreground">{visibleFields.length - 8}</span></summary><div className="grid gap-4 border-t border-border p-3 md:grid-cols-2"><LetterFields items={visibleFields.slice(8)} values={fields} onChange={change} canGenerate={canGenerateField} generatingKey={generatingField} onGenerate={(field) => void generateField(field)} /></div></details>}</div></section>
      {template.salary_rows.length > 0 && <details className="rounded-2xl border border-border bg-card"><summary className="flex cursor-pointer list-none items-center gap-2 p-4 text-sm font-medium">Compensation <span className="text-xs font-normal text-muted-foreground">{template.salary_rows.length}</span><InfoTip label="Compensation fields">CTC and total rows are calculated automatically. Enable Manual overwrite only when HR needs to replace a calculated amount. Clearing an overwritten total restores automatic calculation. Blank component cells count as zero and print as NIL. Rows added or deleted here are applied to the letter annexure the same way.</InfoTip></summary><div className="border-t border-border p-4 md:p-5"><label className="mb-4 flex cursor-pointer items-center justify-between gap-4 rounded-xl border border-border bg-muted/30 p-3"><span><span className="block text-sm font-medium">Manual overwrite</span><span className="mt-0.5 block text-xs text-muted-foreground">Allow editing of automatically calculated CTC and total rows.</span></span><input type="checkbox" checked={manualSalaryOverwrite} onChange={(event) => { setManualSalaryOverwrite(event.target.checked); clearPreview() }} className="size-4 accent-primary" /></label><div className="max-h-[32rem] overflow-auto rounded-xl border border-border"><table className="w-full min-w-[600px] text-sm"><thead className="sticky top-0 bg-muted"><tr><th className="p-3 text-left">Salary component</th>{salaryColumns.map((column) => <th key={column.key} className="p-3 text-left">{column.label}</th>)}</tr></thead><tbody>{annexureRows.map((row) => { const calculated = Boolean(SALARY_FORMULAS[activeKey]?.[row.key]); return <tr key={row.key} className={`border-t border-border ${calculated ? 'bg-primary/[.04]' : ''}`}><td className="p-3 text-xs"><span>{row.label}</span>{calculated && <span className="ml-2 rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-semibold text-primary">AUTO</span>}<button type="button" onClick={() => deleteSalaryRow(row.key)} aria-label={`Delete row ${row.label}`} className="ml-2 inline-flex items-center rounded-md p-1 text-muted-foreground transition hover:bg-destructive/10 hover:text-destructive"><Trash2 className="size-3.5" /></button></td>{salaryColumns.map((column) => <td key={column.key} className="p-2">{row.columns.includes(column.key) ? <input value={resolvedSalaryValues[`${activeScope}:${row.key}:${column.key}`] ?? ''} readOnly={calculated && !manualSalaryOverwrite} onChange={(event) => { setSalaryValues((current) => ({ ...current, [`${activeScope}:${row.key}:${column.key}`]: event.target.value })); clearPreview() }} className={`h-9 w-full rounded-lg border border-border px-2 text-sm ${calculated && !manualSalaryOverwrite ? 'cursor-not-allowed bg-muted font-semibold text-primary' : 'bg-background'}`} placeholder={calculated ? 'Auto-calculated' : 'NIL when blank'} /> : <span className="block text-center text-muted-foreground">—</span>}</td>)}</tr>})}</tbody></table></div>{deletedSalaryRowKeys.length > 0 && <div className="mt-3 flex flex-wrap items-center gap-2 rounded-xl border border-dashed border-border p-3"><span className="text-xs text-muted-foreground">Deleted rows:</span>{deletedSalaryRowKeys.map((key) => { const deletedRow = template?.salary_rows.find((item) => item.key === key); return <button key={key} type="button" onClick={() => restoreSalaryRow(key)} className="inline-flex items-center gap-1 rounded-full border border-border px-2.5 py-1 text-xs transition hover:border-primary/40 hover:text-primary">{deletedRow?.label ?? key}<Undo2 className="size-3" /></button> })}</div>}<div className="mt-3 flex flex-col gap-2 sm:flex-row"><input value={newSalaryRowLabel} onChange={(event) => setNewSalaryRowLabel(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { addSalaryRow(newSalaryRowLabel); setNewSalaryRowLabel('') } }} placeholder="New component name, e.g. Food Allowance" className="h-10 flex-1 rounded-xl border border-border bg-background px-3 text-sm" /><Button type="button" variant="outline" onClick={() => { addSalaryRow(newSalaryRowLabel); setNewSalaryRowLabel('') }} disabled={!newSalaryRowLabel.trim()}><Plus className="mr-1.5 h-4 w-4" />Add row</Button></div></div></details>}
